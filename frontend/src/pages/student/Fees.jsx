  ...
  return (
    <div className="space-y-5 animate-page">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        {[
          { label: 'Total Amount', value: `₹${(data?.total_amount || 0).toLocaleString()}`, icon: DollarSign, bg: 'bg-slate-100 dark:bg-slate-700', iconColor: 'text-slate-600 dark:text-slate-300' },
      ...
      {total > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="card flex flex-col items-center justify-center">
            <h3 className="font-semibold text-slate-900 dark:text-white mb-3 text-sm uppercase tracking-wide self-start">Payment Summary</h3>
          ...
          <div className="card lg:col-span-2">
            <h3 className="font-semibold text-slate-900 dark:text-white mb-4 text-sm uppercase tracking-wide">Payment Progress</h3>
            <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              {allFees.reduce((acc, f) => {
                ...
                <div key={type} className="p-3 bg-slate-50 dark:bg-slate-700 rounded-xl">
                  <p className="text-xs text-slate-500 uppercase tracking-wide">{type}</p>
                  <p className="font-bold text-sm text-slate-900 dark:text-white mt-1">₹{p.toLocaleString()}<span className="text-slate-400 font-normal"> / ₹{t.toLocaleString()}</span></p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="card">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <h3 className="font-semibold text-slate-900 dark:text-white text-sm uppercase tracking-wide">Fee Records</h3>
          <div className="flex gap-2">
            {['all', 'paid', 'unpaid', 'overdue'].map(s => (
              <button key={s} onClick={() => setFilter(s)}
                className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filter === s ? 'bg-primary-600 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'}`}>
                {s === 'all' ? 'All' : s}
              </button>
            ))}
          </div>
        ...
        <div className="space-y-3">
          {filtered.map(f => (
            <div key={f.id} className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-2xl border border-slate-100 dark:border-slate-600">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm text-slate-900 dark:text-white">{f.fee_type}</p>
              ...
              <div className="text-right flex-shrink-0">
                <p className="font-bold text-slate-900 dark:text-white">₹{f.amount.toLocaleString()}</p>
